...
const DoingForm = () => {
    return (
      <div className='flex justify-stretch  flex-col w-full h-dvh border-b-4 border-yellow-950 m-0 p-0'>
      <div className="flex justify-stretch flex-col w-full">
        ...
        <button type="submit" className="btn ml-36 mr-36 mt-4 mb-0 pb-0 border-0 bg-yellow-800 rounded-full">add a verb</button>
        </form>
      </div>
      <div className="flex flex-col justify-stretch w-full mb-24">
        <button onClick={() => DoingDelete(verb)} className=" btn  ml-36 mr-36 mt-0 pt-0 border-0 bg-yellow-800 rounded-full">remove a verb</button>
      </div>
    </div>
    )
}
